...
export class RealChatComponent {
  ...
  ngOnInit() {
    this.user = sessionStorage.getItem('userChat')
      ? JSON.parse(sessionStorage.getItem('userChat'))
      : null;
    ...
    if (sessionStorage.getItem('chatMobileChange')) {
      this.chatMobileChange = JSON.parse(
        sessionStorage.getItem('chatMobileChange')
      );
  ...
  private initPusher() {
    ...
    this.pusher = new Pusher('1592f4180794c697061f', {
      cluster: 'eu',
      authEndpoint:
        'https://c.jordanwebmaster.com/flexihire/public/api/broadcasting/auth',
      auth: {
        headers: {
  ...
  onchatMobileChange() {
    ...
    sessionStorage.setItem(
      'chatMobileChange',
      JSON.stringify(this.chatMobileChange)
    );
  }
}
